...
const DEFAULT_PROJECT_ID = "jgs8u5dy";
...
export async function query<T>(
  ...
  const response = await fetch(url, {
    ...
    // entry expires long before the next build: `force-cache` here would store
    // the response in .next/cache with a one-year TTL, so any CI step that
    // restores that directory between runs would make every rebuild triggered
    // by a publication re-serve the content of the first build.
    //
